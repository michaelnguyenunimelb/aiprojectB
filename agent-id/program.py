...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        ...
        best_move = iterative_deepening(self.board, self.frogs, cellcolor, 6)
        self.time += time.time() - start_time

        print("AGENT ID: ", self.time)
...
def iterative_deepening(board, frogs, color, max_depth):
    storedmove = []
    for depth in range(1, max_depth+1):
        storedmove = []
        minimax(board, frogs, color, depth, -float("inf"), float("inf"), storedmove)
