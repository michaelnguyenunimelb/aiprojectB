...
class Agent:
    ...
    def __init__(self, color: PlayerColor, **referee: dict):
        ...
        for c in range(5):
            self.board[0][c] = Cell.BLUE
            self.board[BOARD_N-1][c] = Cell.RED
            self.frogs[Cell.BLUE].add((0,c))
            self.frogs[Cell.RED].add((BOARD_N-1,c))
        self.board[1][0] = Cell.RED
        self.board[BOARD_N-2][0] = Cell.BLUE
        self.frogs[Cell.RED].add((1,0))
        self.frogs[Cell.BLUE].add((BOARD_N-2,0))
    ...
    def action(self, **referee: dict) -> Action:
        ...
        best_move = self.iterative_deepening(10)
        self.time += time.time() - start_time

        print("AGENT dfs: ", self.time)
    ...
    def dfs_eval(self, color, depth, ply, storemove=None) -> float:
