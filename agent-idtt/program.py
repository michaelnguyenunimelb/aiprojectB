...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        ...
        best_move = iterative_deepening(self.board, self.frogs, cellcolor, 6, self.tt)
        self.time += time.time() - start_time

        print("AGENT IDT: ", self.time)
...
def minimax(board, frogs, color, depth, alpha, beta, tt, storebest=None):
    hashed_int = hash(board, color)
    index = hashed_int % TT_SIZE
    if tt[index] != None and tt[index].key == hashed_int and \
            tt[index].depth >= depth:
        if tt[index].flag == Flag.EXACT:
            return tt[index].val
        if tt[index].flag == Flag.LESS and tt[index].val <= alpha:
            return alpha
        if tt[index].flag == Flag.MORE and tt[index].val >= beta:
            return beta
    ...
    if color == Cell.RED:
        ...
        for m in moves:
            ...
            new_eval = minimax(board, frogs, switch_color(color), depth-1, alpha, beta, tt)
...
def iterative_deepening(board, frogs, color, max_depth, tt):
    storedmove = []
    for depth in range(1, max_depth+1):
        storedmove = []
        minimax(board, frogs, color, depth, -float("inf"), float("inf"), tt, storebest=storedmove)
    return storedmove[0]
