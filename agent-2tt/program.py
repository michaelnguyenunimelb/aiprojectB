...
class Agent:
    ...
    def __init__(self, color: PlayerColor, **referee: dict):
        ...
        self.time = 0
        self.ttr = [None] * TT_SIZE
        self.ttd = [None] * TT_SIZE
    ...
    def action(self, **referee: dict) -> Action:
        ...
        best_move = iterative_deepening(self.board, self.frogs, cellcolor, 6, self.ttr, self.ttd)
        self.time += time.time() - start_time

        print("AGENT 2T: ", self.time)
...
def prio(move, bestmove, secondmove):
    if is_same_move(move, bestmove):
        return BIG_NUM
    if is_same_move(move, secondmove):
        return BIG_NUM - 1
    return move.prio

def minimax(board, frogs, color, depth, alpha, beta, ttr, ttd, storebest=None):
    hashed_int = hash(board, color)
    index = hashed_int % TT_SIZE
    if ttd[index] != None and ttd[index].key == hashed_int and ttd[index].depth >= depth:
        if ttd[index].flag == Flag.EXACT:
            return ttd[index].val
        if ttd[index].flag == Flag.LESS and ttd[index].val <= alpha:
            return alpha
        if ttd[index].flag == Flag.MORE and ttd[index].val >= beta:
            return beta
        
    if ttr[index] != None and ttr[index].key == hashed_int and ttr[index].depth >= depth:
        if ttr[index].flag == Flag.EXACT:
            return ttr[index].val
        if ttr[index].flag == Flag.LESS and ttr[index].val <= alpha:
            return alpha
        if ttr[index].flag == Flag.MORE and ttr[index].val >= beta:
            return beta
    
    if depth == 0:
        val = simple_eval(board, frogs)
        ttr[index] = TTEntry(hashed_int, depth, val, Flag.EXACT, None)
        if ttd[index] == None:
            ttd[index] = TTEntry(hashed_int, depth, val, Flag.EXACT, None)
        return val
    ...
    moverecent = None
    movedeep = None
    if ttr[index] != None and ttr[index].key == hashed_int:
        moverecent = ttr[index].move
    if ttd[index] != None and ttd[index].key == hashed_int:
        movedeep = ttd[index].move
    
    moves.sort(key=lambda x: -prio(x,movedeep, moverecent))
    ...
    if color == Cell.RED:
        ...
        for m in moves:
            ...
            new_eval = minimax(board, frogs, switch_color(color), depth-1, alpha, beta, ttr, ttd)
    ...
    ttr[index] = TTEntry(hashed_int, depth, best_eval, flag, best_move)
    if ttd[index] != None and ttd[index].depth < depth:
        ttd[index] = TTEntry(hashed_int, depth, best_eval, flag, best_move)
    return best_eval

def iterative_deepening(board, frogs, color, max_depth, ttr, ttd):
    storedmove = []
    for depth in range(1, max_depth+1):
        storedmove = []
        minimax(board, frogs, color, depth, -float("inf"), float("inf"), ttr, ttd, storebest=storedmove)
...
def is_same_move(m1, m2):
    if m2 == None:
        return False
    if isinstance(m1, GrowMove) and isinstance(m2, GrowMove):
        return True
    if isinstance(m1, FrogMove) and isinstance(m2, FrogMove) and \
        m1.src == m2.src and m1.dst == m2.dst:
        return True 
    return False