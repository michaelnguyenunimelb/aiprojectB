...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        ...
        minimax(self.board, self.frogs, cellcolor, 6, -float("inf"), float("inf"),storedmove)
        self.time += time.time()-start_time

        print("AGENT E2: ", self.time)
...
def simple_eval_side(board, frogs, color):
    ...
    distance = 0
    positional = 0
    for r,c in frogs[switch_color(color)]:
        if color == Cell.RED:
            distance += r
        else:
            distance += BOARD_N-1-r 
    for r,c in frogs[color]:
        center = (BOARD_N-1)/2
        positional += 0.3 * (center - max(abs(center-r), abs(center-c)))
        
    return distance + positional

def simple_eval(board, frogs):
    redeval = simple_eval_side(board, frogs, Cell.RED) 
    blueeval = simple_eval_side(board, frogs, Cell.BLUE)
    return redeval/(redeval + blueeval)
...
def minimax(board, frogs, color, depth, alpha, beta, storebest=None):
    redwins, bluewins = True, True
    for r,c in frogs[Cell.RED]:
        if r < 7:
            redwins = False
            break
    for r,c in frogs[Cell.BLUE]:
        if r > 0:
            bluewins = False
            break 
    if redwins:
